    ...
    def from_json(cls, data: dict[str, Any]) -> "Variable":
        ...
        if var_type == "text":
            if not isinstance(payload, str):
                raise TypeError("payload for 'text' must be str")
        elif var_type == "json":
            if not isinstance(payload, dict):
                raise TypeError("payload for 'json' must be dict")
        elif var_type == "file":
            if not isinstance(payload, list):
                raise TypeError("payload for 'file' must be list[Attachment]")
            if not all(isinstance(item, (FileAttachment, FileDataAttachment, UrlAttachment)) for item in payload):
                raise TypeError("all 'file' payload items must be Attachment instances")   
...
class DatasetEntryWithRowNo:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "type": self.type,
            "value": value,
            "columnId": self.column_id if self.column_id else None, 
        }
