###########################################
# This file uses langchain 0.1.x
##########################################
...
class TestLoggingUsingLangchain(unittest.TestCase):
    ...
    def test_generation_chat_prompt_azure_chat_model(self):
        ...
        result = model.invoke(
            ...
            config={
                "metadata": {
                    "maxim": {
                        "trace_tags": {"first_tag": "value", "second_tag": "value"}
                    }
                }
            },
        )
    ...
    def test_generation_chat_prompt_bedrock_sonnet_3_chat_mode_streaming(self):
        ...
        for chunk in model.stream(
            messages,
            config={"metadata": {"maxim": {"trace_tags": {"tag1": "tag value"}}}},
        ):
            # print(chunk)
            pass
        logger.flush()
    ...
    def test_generation_chat_prompt_bedrock_sonnet_3_chat_model_without_runnable_sequence_custom_trace(
        self,
    ):
        logger = self.maxim.logger(LoggerConfig(id=repoId))
        #
        # Creating a trace to be used across the agents
        #
        ...
        question = "How are you doing?"
        #
        # attaching input of the trace
        #
        trace.set_input(input=question)

        # Agent 1
        # invoke with passing metadata as agent 3
        ...
        result = chain.invoke(
            ...
            config={"metadata": {"maxim": {"trace_id": trace.id}}},
        )
        #
        # attaching output of the trace
        #
        trace.set_output(output=result)
        #
        # ending trace
        #
        trace.end()
