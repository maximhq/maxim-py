...
# Load config
config_path = os.path.join(os.getcwd(), "maxim", "tests", "testSimulationTestConfig.json")
if not os.path.exists(config_path):
    config_path = os.path.join(os.getcwd(), "testSimulationTestConfig.json")
...
if not config[env].get("apiKey"):
    raise ValueError("Missing API_KEY environment variable")
if not config[env].get("workspaceId"):
    raise ValueError("Missing WORKSPACE_ID environment variable")
if not config[env].get("promptVersionId"):
    raise ValueError("Missing PROMPT_VERSION_ID environment variable")
if not config[env].get("datasetId"):
    raise ValueError("Missing DATASET_ID environment variable")
if not config[env].get("workflowId"):
    raise ValueError("Missing WORKFLOW_ID environment variable")

base_url: str = config[env].get("baseUrl", "https://app.getmaxim.ai")
api_key: str = config[env]["apiKey"]
workspace_id: str = config[env]["workspaceId"]
prompt_version_id: str = config[env]["promptVersionId"]
dataset_id: str = config[env]["datasetId"]
workflow_id: str = config[env]["workflowId"]
