...
class TestPortkeyIntegration(unittest.TestCase):
    def test_instrument_portkey_sync(self):
        client = instrument_portkey(portkey_ai.Portkey(api_key=portkey_api_key,virtual_key=portkey_virtual_key),logger)        
    ...
    def test_portkey_tool_calls_sync(self):
        ...
        instrumented_client = MaximPortkeyClient(portkey_client, logger)
            ...
            if response.choices[0].message.tool_calls:
                ...
                for tool_call in response.choices[0].message.tool_calls:
                    if tool_call.function.name == "get_weather":
                        ...
                        follow_up_response = instrumented_client.chat.completions.create(
                            model="gpt-4o",
                            messages=[
                                {"role": "user", "content": "What's the weather like in San Francisco?"},
                        ...
                        print(f"Follow-up response: {follow_up_response.choices[0].message.content}")
    ...
    def test_portkey_multiple_tool_calls(self):
        ...
        try:
            response = instrumented_client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "user", "content": "Calculate tip for a $50 bill with 20% tip"}
                ],
                tools=tools,
            ...
            if response.choices[0].message.tool_calls:
                ...
                args = json.loads(tool_call.function.arguments)
                tip_amount = args.get("bill_amount", 0) * (args.get("tip_percentage", 0) / 100)
    ...
    async def test_portkey_tool_calls_async(self):
        ...
        instrumented_client = MaximPortkeyClient(async_portkey_client, logger)
        ...
        try:
            response = await instrumented_client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "user", "content": "Search for user information for john.doe@example.com"}
                ],
                tools=tools,
                tool_choice="auto",
                max_tokens=100,
