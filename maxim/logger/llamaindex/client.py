...
_agent_spans: contextvars.ContextVar[Dict[str, Span]] = (
    contextvars.ContextVar("maxim_agent_spans_context_var", default={})
)
_current_generation: contextvars.ContextVar[Dict[str, Generation]] = (
    contextvars.ContextVar("maxim_current_generation_context_var", default={})
)
                    ...
                    async for event in handler.stream_events():
                        ...
                        if hasattr(event, "current_agent_name"):
                            ...
                            if agent_name not in agent_spans:
                                ...
                                agent_spans[agent_name] = trace.span({
                                    ...
                                    "tags": {
                                        "agent_type": (
                                            trace_tags["agent_type"]
                                            if "agent_type" in trace_tags
                                            else "unknown"
                                        )
                                    },
                                })
                        ...
                        if isinstance(event, AgentInput):
                            ...
                            if current_span:
                                ...
                                try:
                                    ...
                                    current_generations = _current_generation.get()
                        ...
                        # Handle agent outputs
                            ...
                            if current_gen:
                                ...
                                current_generations = _current_generation.get()
                                current_generations = current_generations.copy()
                                del current_generations[event.current_agent_name]
                                _current_generation.set(current_generations)
                        ...
                        # Handle tool calls
                            ...
                            if current_gen:
                                ...
                                if current_agent is not None:
                                    current_generations = _current_generation.get()
                                    current_generations = current_generations.copy()
                                    del current_generations[current_agent]
                                    _current_generation.set(current_generations)
                                # Reset token handler for next agent
                                _token_handler.reset_counts()
                                scribe().debug("[MaximSDK] Generation completed and cleaned up")
