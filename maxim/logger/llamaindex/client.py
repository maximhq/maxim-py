...
def instrument_llamaindex(logger: Logger, debug: bool = False):
    ...
    def make_maxim_wrapper(original_method, base_op_name):
        ...
        async def maxim_wrapper(self, *args, **kwargs):
            ...
            if isinstance(self, (AgentWorkflow, FunctionAgent)) and _global_maxim_trace.get() is None:
                trace_id = str(uuid4())
                ...
                trace = logger.trace({
                    ...
                    "tags": {
                        "workflow_type": "agent_workflow" if isinstance(self, AgentWorkflow) else "function_agent",
                        "root_agent": getattr(self, "root_agent", "unknown"), # TODO: This is not correct for FunctionAgent
                    }
                })
            ...
            try:
                ...
                # Set up event handling for workflow
                if isinstance(self, (AgentWorkflow, FunctionAgent)):
                        ...
                        if hasattr(event, "current_agent_name"):
                            ...
                            if agent_name not in _agent_spans:
                                ...
                                _agent_spans[agent_name] = trace.span({
                                    ...
                                    "tags": {"agent_type": "function_agent"}
                                })
                        ...
                        # Handle agent outputs
                            ...
                            if current_gen:
                                if event.response.content and event.raw:
                                    print(f"[MaximSDK] Agent output: {event.raw}")
                                    print(f"[MaximSDK] Agent usage: {event.response.additional_kwargs.get('usage', 0)}")

                                    current_gen.result({
                                        "id": event.raw.get("id", str(uuid4())),
                                        "usage": event.response.additional_kwargs.get("usage", None),
                                        "choices": [
                                            {
                                                "index": 0,
                                                "message": {
                            ...
                            if current_gen:
                                if event.response.content and event.raw:
                                    current_gen.result({
                                        "id": event.raw.get("id", str(uuid4())),
                                        "usage": {
                                            "prompt_tokens": event.response.additional_kwargs.get("prompt_tokens", 0),
                                            "completion_tokens": event.response.additional_kwargs.get("completion_tokens", 0),
                                            "total_tokens": event.response.additional_kwargs.get("total_tokens", 0),
                                        },
                                        "choices": [
                                            {
                                                "index": 0,
                        ...
                        # Handle tool results
                            ...
                            if current_agent is not None:
                                ...
                                if current_span:
                                    ...
                                    try:
                                        tool_call_result_dict = ast.literal_eval(event.tool_output.content)
