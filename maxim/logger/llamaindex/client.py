...
def instrument_llamaindex(logger: Logger, debug: bool = False):
    ...
    def make_maxim_wrapper(original_method, base_op_name):
        ...
        # Create a new callback manager for the workflow
        token_counting_handler = TokenCountingHandler()
        Settings.callback_manager = CallbackManager([token_counting_handler])
                        ...
                        if hasattr(event, "current_agent_name"):
                            ...
                            if agent_name not in _agent_spans:
                                ...
                                _agent_spans[agent_name] = trace.span({
                                    ...
                                    "tags": {"agent_type": trace_tags["agent_type"]}
                                })
                        ...
                        # Handle agent outputs
                            ...
                            if current_gen:
                                if event.response.content:
                                    print(f"[MaximSDK] Agent usage: {token_counting_handler.total_llm_token_count}")
                                    
                                    raw_response = event.raw or {}
                                    current_gen.result({
                                        "id": raw_response.get("id", str(uuid4())),
                                        "usage": {
                                            "prompt_tokens": token_counting_handler.prompt_llm_token_count,
                                            "completion_tokens": token_counting_handler.completion_llm_token_count,
                                            "total_tokens": token_counting_handler.total_llm_token_count,
                                        },
                                        "choices": [
                                            {
                                                "index": 0,
