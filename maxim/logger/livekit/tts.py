...
def handle_tts_synthesize(self, text, voice=None):
    """Handle TTS synthesis with text input"""
    try:
        ...
        if session_info is None:
            scribe().debug("[Internal][TTS] No session info found for TTS synthesis")
            return

        turn = session_info.current_turn
        if turn is None:
            scribe().debug("[Internal][TTS] No current turn for TTS synthesis")
            return
        ...
        # Create generation for TTS
        trace = get_session_store().get_current_trace_for_agent_session(
            id(self._session)
        )
        if trace is not None:
            # Create TTS generation
            tts_gen_id = str(uuid4())
            trace.generation(
                {
                    "id": tts_gen_id,
                    "model": getattr(self, "_model", "unknown-tts"),
                    "name": "TTS Synthesis",
                    "provider": "openai",
                    "model_parameters": {"voice": voice} if voice else {},
                    "messages": [{"role": "assistant", "content": text}],
                }
            )

            # Store generation ID for later audio attachment
            turn.tts_generation_id = tts_gen_id
        ...
        scribe().warning(
            f"[Internal][TTS] Text handling failed: {e!s}\n{traceback.format_exc()}"
        )
...
def handle_tts_result(self, result):
    ...
    try:
        ...
        if audio_data:
            ...
            # Add audio attachment to TTS generation if we have generation ID
            if hasattr(turn, "tts_generation_id") and turn.tts_generation_id:
                get_maxim_logger().generation_add_attachment(
                    turn.tts_generation_id,
                    FileDataAttachment(
...
def pre_hook(self, hook_name, args, kwargs):
    ...
    try:
        if hook_name in ["synthesize", "asynthesize", "speak"]:
            ...
            voice = kwargs.get("voice") or (args[1] if len(args) > 1 else None)
            get_thread_pool_executor().submit(handle_tts_synthesize, self, text, voice)
        else:
            scribe().debug(
